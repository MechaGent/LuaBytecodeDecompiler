...
public class StepStage implements Comparable<StepStage>
{
	...
	public StepStage(int inLineNum, int inSubLineNum, int inStepNum)
	{
		this(inLineNum, inSubLineNum, inStepNum, "");
	}
	...
	public static StepStage[] requestSequence(int line, int subLine, int startStep, int numSteps)
	{
		final StepStage[] result = new StepStage[numSteps];
		
		for(int i = 0; i < numSteps; i++)
		{
			result[i] = new StepStage(line, subLine, startStep + i);
		}
		
		return result;
	}
	
	public StepStage cloneAndIncrementStepNumBy(int delta)
	{
		return new StepStage(this.lineNum, this.subLineNum, this.stepNum + delta, this.stageName);
	}
	...
	public void setStageName(String in)
	{
		this.stageName = in;
	}
